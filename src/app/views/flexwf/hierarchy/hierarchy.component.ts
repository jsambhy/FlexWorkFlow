...
export class HierarchyComponent {
  ...
  @ViewChild('Hierrachygrid', { static: false }) public Hierrachygrid: GridComponent;
  @ViewChild('SelectedUserlistbox', { static: false }) public SelectedUserlistbox: ListBoxComponent;
  hidden: boolean = false;
  HierarchycreateForm: FormGroup = null;
  ...
  showCreateHierarchyPopup: boolean;
  ...
  get f() { return this.HierarchycreateForm.controls; }

  isFieldValid(field: string) {
    return !this.HierarchycreateForm.get(field).valid && (this.HierarchycreateForm.get(field).dirty || this.HierarchycreateForm.get(field).touched);
  }
  ...
  ngOnInit() {
    ...
    this.toolbar =
      [{ text: 'Add', tooltipText: 'Add', prefixIcon: 'e-custom-icons e-action-Icon-add', id: 'Add' }, { text: 'Edit', tooltipText: 'Edit', prefixIcon: 'e-custom-icons e-action-Icon-editpencil', id: 'Edit' }, { text: 'Delete', tooltipText: 'Delete', prefixIcon: 'e-custom-icons e-action-Icon-delete3', id: 'Delete' }, { text: 'WorkFlow Designer', tooltipText: 'WorkFlow Designer', prefixIcon: 'e-custom-icons e-action-Icon-workflowdesigner', id: 'WFDesign' }, { text: 'Data Fields', tooltipText: 'Data Fields', prefixIcon: 'e-custom-icons e-action-Icon-datafields', id: 'FormDesign' }, 'ExcelExport', 'PdfExport', 'CsvExport', 'Search'
      ];
  ...
  toolbarClick(args: ClickEventArgs): void {
    const selectedRecords = this.Hierrachygrid.getSelectedRecords();
    if (args.item.id === 'grid_pdfexport') {
      this.Hierrachygrid.pdfExport();
    }
    if (args.item.id === 'grid_excelexport') {
      this.Hierrachygrid.excelExport();
    }
    if (args.item.id === 'grid_csvexport') {
      this.Hierrachygrid.csvExport();
    }

    if (args.item.id === 'Add') {
      this.HierarchyHeader = "Create Hierarchy";
      /* this.SelectedUsersDataSrc = null;*/
      this.HierarchycreateForm.patchValue({
        Name: null,
        Description: null
      });

      this.showCreateHierarchyPopup = true;
      this.ShowCreateBtn = true;
      this.ShowUpdateBtn = false;

    }
    if (args.item.id === 'Edit') {
      this.HierarchyHeader = "Edit Hierarchy";
      if (selectedRecords.length == 0) {
        this.toasts[3].content = "No hierarchy selected for this operation";
        this.toastObj.show(this.toasts[3]);
      }
      else {
    ...
    if (args.item.id === 'Delete') {
      if (selectedRecords.length == 0) {
        ...
        //this.selectedHierarchyId = selectedRecords[0]['Id'];
        //this.fnDelete();
      }
    }
  }
  ...
  constructor(private formBuilder: FormBuilder) {
    ...
    this.HierarchycreateForm = this.formBuilder.group({
